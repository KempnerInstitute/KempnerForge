...
def _serial_reference(path, fps: float, min_frames: int, max_frames: int) -> tuple[list, list]:
    """Ground-truth ``(frames, times)`` via the serial reference pass (bypasses seeking)."""
    import av
...
class TestSeekMatchesSerial:
    ...
    def _assert_parity(self, path, *, fps, min_frames, max_frames):
        ...
        expected, expected_times = _serial_reference(path, fps, min_frames, max_frames)
        got, got_times = decode_video_frames(
...
class TestSeekMatchesSerialH264:
    ...
    def _assert_parity_seek_ran(self, path, *, fps, min_frames, max_frames, monkeypatch):
        ...
        got, got_times = video_io.decode_video_frames(
            str(path), fps=fps, min_frames=min_frames, max_frames=max_frames
        )
    ...
    def test_explicit_targets_straddle_gop_boundaries(self, tmp_path, open_gop):
        ...
        got, got_times = _run(_decode_seek)
        expected, expected_times = _run(_decode_serial)
        assert len(got) == len(expected)
        assert [f.tobytes() for f in got] == [f.tobytes() for f in expected]
        assert got_times == expected_times
...
class TestSeekFallback:
    ...
    def test_falls_back_to_serial_on_unreliable_seek(self, tmp_path, monkeypatch):
        ...
        path = tmp_path / "clip.mp4"
        _write_mp4(path, n_frames=40, gop_size=12)
        expected, expected_times = _serial_reference(path, 2.0, 4, 8)
        ...
        got, got_times = video_io.decode_video_frames(str(path), fps=2.0, min_frames=4, max_frames=8)
        assert [f.tobytes() for f in got] == [f.tobytes() for f in expected]
        assert got_times == expected_times
