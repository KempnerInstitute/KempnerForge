...
- **Ragged grids in `attentional_pool`.** The `attentional_pool` connector now pools patch grids the window does not evenly divide, instead of rejecting them: the grid is padded to `ceil(grid/window) * window` and each partial edge window pools only its real patches, with the padded patches masked out of that window's K/V and of the mean-query. Reaches parity with `avgpool`, so any window pools any grid (e.g. a 3×3 window on a 14×14 SigLIP2 grid → 5×5 = 25 tokens, previously refused). `output_num_tokens` is `ceil(grid/window)²` for both pooling connectors; a divisible grid builds no mask and is bit-exact with before, so no existing config's token count or loss changes.
  ...
  - Tests: `tests/unit/test_adapter.py` — ragged token counts, equivalence against a brute-force per-window reference, pad-value independence, `grid % window == 1`, `window > grid`, `window == grid`, and non-square inputs still rejected.
  - Docs: `docs/how-to/train-on-video.md`.
- **Config-driven model knobs: decoupled attention head dim, an optional adapter pre-norm, and weights-only warm-start.** All default to today's behavior.
  - `kempnerforge/config/model.py`: `head_dim_override` (`0` -> `dim // n_heads`) decouples the attention width from the residual width, so `n_heads * head_dim` need not equal `dim`. `head_dim` stays a derived property, so a TOML/CLI overlay onto a default config cannot leave a stale value behind. `dim % n_heads == 0` is required only when the head dim is inferred.
  - `kempnerforge/model/cross_attention.py`, `transformer.py`: `CrossAttentionBlock` threads `head_dim` to `CrossAttention`, but only when `head_dim_override` is set -- a coupled model with a custom `vlm.cross_attention_n_heads` keeps its existing `dim // n_heads` width.
  - `kempnerforge/config/adapter.py`, `model/adapter.py`: `adapter.pre_norm` adds a norm over the vision features before `mlp_2layer`'s first projection (exposed as `ln_q`), by norm-registry key. `""` (default) builds no module and leaves the state dict and RNG stream unchanged.
  - `kempnerforge/training/entry.py`: a `load_path` warm start now honors `checkpoint.exclude_from_loading` (previously an inert field), so a weights-only checkpoint fine-tunes cleanly; a real resume ignores it and always restores full state.
- **Config-declared plugin modules** (`plugins`). A top-level list of module paths that `load_config` imports *before* the TOML is overlaid onto the dataclasses, so components registered out-of-tree are in the registry by the time the `__post_init__` validators resolve names against it (`video.dataset_type`, `adapter.type`, `vision_encoder.type`, …). This is the seam that lets an experiment under `examples/` ship its own dataset/adapter/loss without any core module importing it. Default `[]` runs no import machinery, so existing configs are unchanged.
  - `kempnerforge/config/job.py`: `plugins: list[str]` (default empty), rejected in `__post_init__` unless it is a list of strings.
  - `kempnerforge/config/loader.py`: `_import_plugins` — imports each module once per process and raises a `ValueError` naming the module and the config key when one is missing or raises. Reads the key from the raw TOML dict / parsed CLI overrides, so `--plugins=["pkg.mod"]` works like any other field.
  - Tests: `tests/unit/test_config.py::TestConfigPlugins` (ordering, CLI path, error messages, single import, RNG purity, field type) and `tests/unit/test_checkpoint.py::TestComponentSwapOnResume` (a checkpoint does not silently load into a differently built component).
  - Docs: `docs/configuration/registry.md`, plus the `JobConfig` table in `docs/configuration/config-sections.md`.
