...
- CI `unit-tests` job also runs the hermetic example unit tier (`examples/vlm/eval/tests/unit`), which sits outside the root `testpaths`. Each tier gets its own pytest session; the integration tier stays manual (it needs the undeclared `lmms-eval`).
