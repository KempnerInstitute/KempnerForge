...
class MLP2LayerAdapter(VisionAdapter):
    ...
    (e.g. ``"rmsnorm"`` / ``"layernorm"``); ``None``/empty disables it.
    ...
    def __init__(
        ...
        # Optional pre-projection norm over the vision features. Registry-
        # selected, so any registered norm type works. Exposed as ``ln_q``,
        # the conventional name for a merger-style connector's input norm.
