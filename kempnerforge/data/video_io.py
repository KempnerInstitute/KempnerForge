...
def _decode_seek(
    ...
    frame. Returns ``(images, times)`` with ``times`` the matched frames'
    presentation times, parallel to ``images``. Raises
    ``_SeekUnreliableError`` whenever identical selection cannot be
    guaranteed: no ``time_base``, a frame without PTS, a seek landing past
...
def _decode_serial(
    ...
    Returns ``(images, times)`` like ``_decode_seek``; frame times come from
    ``_frame_time``, whose index/avg_rate fallback keeps them monotonic on
    containers that report no per-frame PTS instead of silently all-zero.
    """
    images: list[PILImage] = []
    times: list[float] = []
    j = 0
